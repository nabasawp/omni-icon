...
use OmniIconDeps\Seld\JsonLint\JsonParser;
...
use OmniIconDeps\Symfony\Component\Cache\Adapter\DoctrineAdapter;
use OmniIconDeps\Symfony\Component\Config\Definition\Builder\ArrayNodeDefinition;
use OmniIconDeps\Symfony\Component\Config\Definition\Builder\NodeBuilder;
...
use OmniIconDeps\Symfony\Component\HttpFoundation\Cookie;
...
use OmniIconDeps\Symfony\Component\Lock\Lock;
use OmniIconDeps\Symfony\Component\Lock\Store\SemaphoreStore;
...
use OmniIconDeps\Symfony\Component\RateLimiter\Policy\TokenBucketLimiter;
use OmniIconDeps\Symfony\Component\RemoteEvent\RemoteEvent;
use OmniIconDeps\Symfony\Component\Scheduler\Schedule;
use OmniIconDeps\Symfony\Component\Semaphore\Semaphore;
use OmniIconDeps\Symfony\Component\Serializer\Encoder\JsonDecode;
...
use OmniIconDeps\Symfony\Component\Workflow\Validator\DefinitionValidatorInterface;
...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode->docUrl('https://symfony.com/doc/{version:major}.{version:minor}/reference/configuration/framework.html', 'symfony/framework-bundle')->beforeNormalization()->ifArray()->then(static function ($v) {
            if (isset($v['templating']) && class_exists(Package::class)) {
                $v['assets'] ??= [];
            }
            return $v;
        })->end()->children()->scalarNode('secret')->end()->booleanNode('http_method_override')->info("Set true to enable support for the '_method' request parameter to determine the intended HTTP method on POST requests.")->defaultFalse()->end()->arrayNode('allowed_http_method_override')->info('Sets the list of HTTP methods that can be overridden. Set to null to allow all methods to be overridden (default). Set to an empty array to disallow overrides entirely. Otherwise, provide the list of uppercased method names that are allowed.')->stringPrototype()->end()->defaultNull()->validate()->ifTrue(static fn($v) => array_intersect($v, ['GET', 'HEAD', 'CONNECT', 'TRACE']))->thenInvalid('The HTTP methods "GET", "HEAD", "CONNECT", and "TRACE" cannot be overridden.')->end()->end()->scalarNode('trust_x_sendfile_type_header')->info('Set true to enable support for xsendfile in binary file responses.')->defaultValue('%env(bool:default::SYMFONY_TRUST_X_SENDFILE_TYPE_HEADER)%')->end()->scalarNode('ide')->defaultValue($this->debug ? '%env(default::SYMFONY_IDE)%' : null)->end()->booleanNode('test')->end()->scalarNode('default_locale')->defaultValue('en')->end()->booleanNode('set_locale_from_accept_language')->info('Whether to use the Accept-Language HTTP header to set the Request locale (only when the "_locale" request attribute is not passed).')->defaultFalse()->end()->booleanNode('set_content_language_from_locale')->info('Whether to set the Content-Language HTTP header on the Response using the Request locale.')->defaultFalse()->end()->arrayNode('enabled_locales', 'enabled_locale')->info('Defines the possible locales for the application. This list is used for generating translations files, but also to restrict which locales are allowed when it is set from Accept-Language header (using "set_locale_from_accept_language").')->prototype('scalar')->end()->end()->arrayNode('trusted_hosts')->beforeNormalization()->ifString()->then(static fn($v) => $v ? [$v] : [])->end()->prototype('scalar')->end()->defaultValue(['%env(default::SYMFONY_TRUSTED_HOSTS)%'])->end()->variableNode('trusted_proxies')->beforeNormalization()->ifTrue(static fn($v) => 'private_ranges' === $v || 'PRIVATE_SUBNETS' === $v)->then(static fn() => IpUtils::PRIVATE_SUBNETS)->end()->defaultValue(['%env(default::SYMFONY_TRUSTED_PROXIES)%'])->end()->arrayNode('trusted_headers', 'trusted_header')->performNoDeepMerging()->beforeNormalization()->ifString()->then(static fn($v) => $v ? [$v] : [])->end()->prototype('scalar')->end()->defaultValue(['%env(default::SYMFONY_TRUSTED_HEADERS)%'])->end()->scalarNode('error_controller')->defaultValue('error_controller')->end()->booleanNode('handle_all_throwables')->info('HttpKernel will handle all kinds of \Throwable.')->defaultTrue()->end()->end();
    ...
    {
        $rootNode->children()->arrayNode('secrets')->canBeDisabled()->children()->scalarNode('vault_directory')->defaultValue('%kernel.project_dir%/config/secrets/%kernel.runtime_environment%')->cannotBeEmpty()->end()->scalarNode('local_dotenv_file')->defaultValue('%kernel.project_dir%/.env.%kernel.runtime_environment%.local')->end()->scalarNode('decryption_env_var')->defaultValue('base64:default::SYMFONY_DECRYPTION_SECRET')->end()->end()->end()->end();
    }
    private function addCsrfSection(ArrayNodeDefinition $rootNode): void
    {
        $rootNode->children()->arrayNode('csrf_protection')->treatFalseLike(['enabled' => \false])->treatTrueLike(['enabled' => \true])->treatNullLike(['enabled' => \true])->addDefaultsIfNotSet()->children()->scalarNode('enabled')->defaultNull()->end()->arrayNode('stateless_token_ids', 'stateless_token_id')->scalarPrototype()->end()->info('Enable headers/cookies-based CSRF validation for the listed token ids.')->end()->scalarNode('check_header')->defaultFalse()->info('Whether to check the CSRF token in a header in addition to a cookie when using stateless protection.')->end()->scalarNode('cookie_name')->defaultValue('csrf-token')->info('The name of the cookie to use when using stateless protection.')->end()->end()->end()->end();
    }
    private function addFormSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('form')->info('Form configuration')->{$enableIfStandalone('symfony/form', Form::class)}()->children()->arrayNode('csrf_protection')->treatFalseLike(['enabled' => \false])->treatTrueLike(['enabled' => \true])->treatNullLike(['enabled' => \true])->addDefaultsIfNotSet()->children()->scalarNode('enabled')->defaultNull()->end()->scalarNode('token_id')->defaultNull()->end()->scalarNode('field_name')->defaultValue('_token')->end()->arrayNode('field_attr')->performNoDeepMerging()->normalizeKeys(\false)->useAttributeAsKey('name')->scalarPrototype()->end()->defaultValue(['data-controller' => 'csrf-protection'])->end()->end()->end()->end()->end()->end();
    }
    private function addHttpCacheSection(ArrayNodeDefinition $rootNode): void
    {
        $rootNode->children()->arrayNode('http_cache')->info('HTTP cache configuration')->canBeEnabled()->children()->booleanNode('debug')->defaultValue('%kernel.debug%')->end()->enumNode('trace_level')->values(['none', 'short', 'full'])->end()->scalarNode('trace_header')->end()->integerNode('default_ttl')->end()->arrayNode('private_headers', 'private_header')->performNoDeepMerging()->scalarPrototype()->end()->end()->arrayNode('skip_response_headers', 'skip_response_header')->performNoDeepMerging()->scalarPrototype()->end()->end()->booleanNode('allow_reload')->end()->booleanNode('allow_revalidate')->end()->integerNode('stale_while_revalidate')->end()->integerNode('stale_if_error')->end()->booleanNode('terminate_on_cache_hit')->end()->end()->end()->end();
    }
    ...
    {
        $rootNode->children()->arrayNode('workflows', 'workflow')->canBeEnabled()->beforeNormalization()->ifArray()->then(static function ($v) {
            if (\true === ($v['enabled'] ?? \false)) {
                $workflows = $v;
            ...
            return $v;
        })->end()->children()->arrayNode('workflows', 'workflow')->useAttributeAsKey('name')->prototype('array')->children()->arrayNode('audit_trail')->canBeEnabled()->end()->enumNode('type')->values(['workflow', 'state_machine'])->defaultValue('state_machine')->end()->arrayNode('marking_store')->children()->enumNode('type')->values(['method'])->end()->scalarNode('property')->cannotBeEmpty()->end()->scalarNode('service')->cannotBeEmpty()->end()->end()->end()->arrayNode('supports', 'support')->acceptAndWrap(['string'])->prototype('scalar')->cannotBeEmpty()->validate()->ifTrue(static fn($v) => !class_exists($v) && !interface_exists($v, \false))->thenInvalid('The supported class or interface "%s" does not exist.')->end()->end()->end()->arrayNode('definition_validators', 'definition_validator')->prototype('scalar')->cannotBeEmpty()->validate()->ifTrue(static fn($v) => !class_exists($v))->thenInvalid('The validation class %s does not exist.')->end()->validate()->ifTrue(static fn($v) => !is_a($v, DefinitionValidatorInterface::class, \true))->thenInvalid(\sprintf('The validation class %%s is not an instance of "%s".', DefinitionValidatorInterface::class))->end()->validate()->ifTrue(static fn($v) => 1 <= (new \ReflectionClass($v))->getConstructor()?->getNumberOfRequiredParameters())->thenInvalid('The %s validation class constructor must not have any arguments.')->end()->end()->end()->scalarNode('support_strategy')->cannotBeEmpty()->end()->arrayNode('initial_marking')->acceptAndWrap(['backed-enum', 'string'])->defaultValue([])->beforeNormalization()->ifArray()->then(static function ($markings) {
            $normalizedMarkings = [];
            foreach ($markings as $marking) {
                $normalizedMarkings[] = $marking instanceof \BackedEnum ? $marking->value : $marking;
            }
            return $normalizedMarkings;
        })->end()->prototype('scalar')->end()->end()->arrayNode('events_to_dispatch', 'event_to_dispatch')->defaultNull()->stringPrototype()->end()->validate()->ifTrue(static function ($v) {
            if (!class_exists(WorkflowEvents::class)) {
                return \false;
            }
            foreach ($v as $value) {
                if (!\in_array($value, WorkflowEvents::ALIASES, \true)) {
                    return \true;
                }
            }
            return \false;
        })->thenInvalid('The value must be "null" or an array of workflow events (like ["workflow.enter"]).')->end()->info('Select which Transition events should be dispatched for this Workflow.')->example(['workflow.enter', 'workflow.transition'])->end()->arrayNode('places', 'place')->beforeNormalization()->ifString()->then(static function ($places) {
        ...
        })->end()->beforeNormalization()->ifArray()->then(static function ($places) {
            $normalizedPlaces = [];
        ...
        })->end()->prototype('array')->children()->scalarNode('name')->isRequired()->cannotBeEmpty()->end()->arrayNode('metadata')->normalizeKeys(\false)->defaultValue([])->example(['color' => 'blue', 'description' => 'Workflow to manage article.'])->prototype('variable')->end()->end()->end()->end()->end()->arrayNode('transitions', 'transition')->beforeNormalization()->ifArray()->then(static function ($transitions) {
        ...
        })->end()->isRequired()->requiresAtLeastOneElement()->prototype('array')->children()->stringNode('name')->isRequired()->cannotBeEmpty()->end()->stringNode('guard')->cannotBeEmpty()->info('An expression to block the transition.')->example('is_fully_authenticated() and is_granted(\'ROLE_JOURNALIST\') and subject.getTitle() == \'My first article\'')->end()->arrayNode('from')->performNoDeepMerging()->acceptAndWrap(['backed-enum', 'string'])->beforeNormalization()->ifArray()->then($workflowNormalizeArcs = static function ($arcs) {
        ...
        })->end()->requiresAtLeastOneElement()->prototype('array')->children()->stringNode('place')->isRequired()->cannotBeEmpty()->end()->integerNode('weight')->defaultValue(1)->min(1)->end()->end()->end()->end()->arrayNode('to')->performNoDeepMerging()->acceptAndWrap(['backed-enum', 'string'])->beforeNormalization()->ifArray()->then($workflowNormalizeArcs)->end()->requiresAtLeastOneElement()->prototype('array')->children()->stringNode('place')->isRequired()->cannotBeEmpty()->end()->integerNode('weight')->defaultValue(1)->min(1)->end()->end()->end()->end()->integerNode('weight')->defaultValue(1)->validate()->ifTrue(static fn($v) => $v < 1)->thenInvalid('The weight must be greater than 0.')->end()->end()->arrayNode('metadata')->normalizeKeys(\false)->defaultValue([])->example(['color' => 'blue', 'description' => 'Workflow to manage article.'])->prototype('variable')->end()->end()->end()->end()->end()->arrayNode('metadata')->normalizeKeys(\false)->defaultValue([])->example(['color' => 'blue', 'description' => 'Workflow to manage article.'])->prototype('variable')->end()->end()->end()->validate()->ifTrue(static fn($v) => $v['supports'] && isset($v['support_strategy']))->thenInvalid('"supports" and "support_strategy" cannot be used together.')->end()->validate()->ifTrue(static fn($v) => !$v['supports'] && !isset($v['support_strategy']))->thenInvalid('"supports" or "support_strategy" should be configured.')->end()->beforeNormalization()->ifArray()->then(static function ($values) {
    ...
    private function addAssetsSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        ...
        })->thenInvalid('You cannot use both "version" and "json_manifest_path" at the same time under "assets".')->end()->children()->arrayNode('packages', 'package')->normalizeKeys(\false)->useAttributeAsKey('name')->prototype('array')->children()->booleanNode('strict_mode')->info('Throw an exception if an entry is missing from the manifest.json.')->defaultFalse()->end()->scalarNode('version_strategy')->defaultNull()->end()->scalarNode('version')->beforeNormalization()->ifString()->then(static fn($v) => '' === $v ? null : $v)->end()->end()->scalarNode('version_format')->defaultNull()->end()->scalarNode('json_manifest_path')->defaultNull()->end()->scalarNode('base_path')->defaultValue('')->end()->arrayNode('base_urls', 'base_url')->requiresAtLeastOneElement()->acceptAndWrap(['string'])->prototype('scalar')->end()->end()->end()->validate()->ifTrue(function ($v) {
            return isset($v['version_strategy']) && isset($v['version']);
    ...
    {
        $rootNode->children()->arrayNode('asset_mapper')->info('Asset Mapper configuration')->{$enableIfStandalone('symfony/asset-mapper', AssetMapper::class)}()->children()->arrayNode('paths', 'path')->info('Directories that hold assets that should be in the mapper. Can be a simple array of an array of ["path/to/assets": "namespace"].')->example(['assets/'])->normalizeKeys(\false)->useAttributeAsKey('namespace')->acceptAndWrap(['string'])->beforeNormalization()->ifArray()->then(static function ($v) {
            $result = [];
            ...
            return $result;
        })->end()->prototype('scalar')->end()->end()->arrayNode('excluded_patterns', 'excluded_pattern')->info('Array of glob patterns of asset file paths that should not be in the asset mapper.')->prototype('scalar')->end()->example(['*/assets/build/*', '*/*_.scss'])->end()->booleanNode('exclude_dotfiles')->info('If true, any files starting with "." will be excluded from the asset mapper.')->defaultTrue()->end()->booleanNode('server')->info('If true, a "dev server" will return the assets from the public directory (true in "debug" mode only by default).')->defaultValue($this->debug)->end()->scalarNode('public_prefix')->info('The public path where the assets will be written to (and served from when "server" is true).')->defaultValue('/assets/')->end()->enumNode('missing_import_mode')->values(['strict', 'warn', 'ignore'])->info('Behavior if an asset cannot be found when imported from JavaScript or CSS files - e.g. "import \'./non-existent.js\'". "strict" means an exception is thrown, "warn" means a warning is logged, "ignore" means the import is left as-is.')->defaultValue('warn')->end()->arrayNode('extensions', 'extension')->info('Key-value pair of file extensions set to their mime type.')->normalizeKeys(\false)->useAttributeAsKey('extension')->example(['.zip' => 'application/zip'])->prototype('scalar')->end()->end()->scalarNode('importmap_path')->info('The path of the importmap.php file.')->defaultValue('%kernel.project_dir%/importmap.php')->end()->scalarNode('importmap_polyfill')->info('The importmap name that will be used to load the polyfill. Set to false to disable.')->validate()->ifTrue()->thenInvalid('Invalid "importmap_polyfill" value. Must be either an importmap name or false.')->end()->defaultValue('es-module-shims')->end()->arrayNode('importmap_script_attributes', 'importmap_script_attribute')->info('Key-value pair of attributes to add to script tags output for the importmap.')->normalizeKeys(\false)->useAttributeAsKey('key')->example(['data-turbo-track' => 'reload'])->prototype('scalar')->end()->end()->scalarNode('vendor_dir')->info('The directory to store JavaScript vendors.')->defaultValue('%kernel.project_dir%/assets/vendor')->end()->arrayNode('precompress')->info('Precompress assets with Brotli, Zstandard and gzip.')->canBeEnabled()->children()->arrayNode('formats', 'format')->info('Array of formats to enable. "brotli", "zstandard" and "gzip" are supported. Defaults to all formats supported by the system. The entire list must be provided.')->prototype('scalar')->end()->performNoDeepMerging()->validate()->ifTrue(static fn($v) => array_diff($v, ['brotli', 'zstandard', 'gzip']))->thenInvalid('Unsupported format: "brotli", "zstandard" and "gzip" are supported.')->end()->end()->arrayNode('extensions', 'extension')->info('Array of extensions to compress. The entire list must be provided, no merging occurs.')->prototype('scalar')->end()->performNoDeepMerging()->defaultValue(interface_exists(CompressorInterface::class) ? CompressorInterface::DEFAULT_EXTENSIONS : [])->end()->end()->end()->end()->end()->end();
    }
    private function addTranslatorSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('translator')->info('Translator configuration')->{$enableIfStandalone('symfony/translation', Translator::class)}()->children()->arrayNode('fallbacks', 'fallback')->info('Defaults to the value of "default_locale".')->acceptAndWrap(['string'])->prototype('scalar')->end()->defaultValue([])->end()->booleanNode('logging')->defaultFalse()->end()->scalarNode('formatter')->defaultValue('translator.formatter.default')->end()->scalarNode('cache_dir')->defaultValue('%kernel.cache_dir%/translations')->end()->scalarNode('default_path')->info('The default path used to load translations.')->defaultValue('%kernel.project_dir%/translations')->end()->arrayNode('paths', 'path')->prototype('scalar')->end()->end()->arrayNode('pseudo_localization')->canBeEnabled()->children()->booleanNode('accents')->defaultTrue()->end()->floatNode('expansion_factor')->min(1.0)->defaultValue(1.0)->end()->booleanNode('brackets')->defaultTrue()->end()->booleanNode('parse_html')->defaultFalse()->end()->arrayNode('localizable_html_attributes', 'localizable_html_attribute')->prototype('scalar')->end()->end()->end()->end()->arrayNode('providers', 'provider')->info('Translation providers you can read/write your translations from.')->useAttributeAsKey('name')->prototype('array')->children()->scalarNode('dsn')->end()->arrayNode('domains', 'domain')->prototype('scalar')->end()->defaultValue([])->end()->arrayNode('locales', 'locale')->prototype('scalar')->end()->defaultValue([])->info('If not set, all locales listed under framework.enabled_locales are used.')->end()->end()->end()->defaultValue([])->end()->arrayNode('globals', 'global')->info('Global parameters.')->example(['app_version' => 3.14])->normalizeKeys(\false)->useAttributeAsKey('name')->arrayPrototype()->acceptAndWrap(['string'], 'value')->children()->variableNode('value')->end()->stringNode('message')->end()->arrayNode('parameters', 'parameter')->normalizeKeys(\false)->useAttributeAsKey('name')->scalarPrototype()->end()->end()->stringNode('domain')->end()->end()->validate()->ifTrue(static fn($v) => !(isset($v['value']) xor isset($v['message'])))->thenInvalid('The "globals" parameter should be either a string or an array with a "value" or a "message" key')->end()->end()->end()->end()->end()->end();
    }
    private function addValidationSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('validation')->info('Validation configuration')->{$enableIfStandalone('symfony/validator', Validation::class)}()->children()->scalarNode('cache')->setDeprecated('symfony/framework-bundle', '7.3', 'Setting the "%path%.%node%" configuration option is deprecated. It will be removed in version 8.0.')->end()->booleanNode('enable_attributes')->{class_exists(FullStack::class) ? 'defaultFalse' : 'defaultTrue'}()->end()->arrayNode('static_method')->acceptAndWrap(['string'])->defaultValue(['loadValidatorMetadata'])->prototype('scalar')->end()->treatFalseLike([])->end()->scalarNode('translation_domain')->defaultValue('validators')->end()->enumNode('email_validation_mode')->values(['html5', 'html5-allow-no-tld', 'strict', 'loose'])->defaultValue('html5')->end()->arrayNode('mapping')->addDefaultsIfNotSet()->children()->arrayNode('paths', 'path')->prototype('scalar')->end()->end()->end()->end()->arrayNode('not_compromised_password')->canBeDisabled('When disabled, compromised passwords will be accepted as valid.')->children()->scalarNode('endpoint')->defaultNull()->info('API endpoint for the NotCompromisedPassword Validator.')->end()->end()->end()->booleanNode('disable_translation')->defaultFalse()->end()->arrayNode('auto_mapping')->info('A collection of namespaces for which auto-mapping will be enabled by default, or null to opt-in with the EnableAutoMapping constraint.')->example(['App\Entity\\' => [], 'App\WithSpecificLoaders\\' => ['validator.property_info_loader']])->useAttributeAsKey('namespace')->normalizeKeys(\false)->beforeNormalization()->ifArray()->then(static function ($values) {
            ...
            return $values;
        })->end()->arrayPrototype()->children()->arrayNode('services', 'service')->prototype('scalar')->end()->end()->end()->end()->end()->end()->end()->end();
    }
    private function addAnnotationsSection(ArrayNodeDefinition $rootNode): void
    {
        $rootNode->children()->arrayNode('annotations')->canBeEnabled()->validate()->ifTrue(static fn($v) => $v['enabled'])->thenInvalid('Enabling the doctrine/annotations integration is not supported anymore.')->end()->end();
    }
    private function addSerializerSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $defaultContextNode = fn() => (new NodeBuilder())->arrayNode('default_context')->normalizeKeys(\false)->validate()->ifTrue(fn() => $this->debug && class_exists(JsonParser::class))->then(fn(array $v) => $v + [JsonDecode::DETAILED_ERROR_MESSAGES => \true])->end()->defaultValue([])->prototype('variable')->end();
        $rootNode->children()->arrayNode('serializer')->info('Serializer configuration')->{$enableIfStandalone('symfony/serializer', Serializer::class)}()->children()->booleanNode('enable_attributes')->{class_exists(FullStack::class) ? 'defaultFalse' : 'defaultTrue'}()->end()->scalarNode('name_converter')->end()->scalarNode('circular_reference_handler')->end()->scalarNode('max_depth_handler')->end()->arrayNode('mapping')->addDefaultsIfNotSet()->children()->arrayNode('paths', 'path')->prototype('scalar')->end()->end()->end()->end()->append($defaultContextNode())->arrayNode('named_serializers', 'named_serializer')->useAttributeAsKey('name')->arrayPrototype()->children()->scalarNode('name_converter')->end()->append($defaultContextNode())->booleanNode('include_built_in_normalizers')->info('Whether to include the built-in normalizers')->defaultTrue()->end()->booleanNode('include_built_in_encoders')->info('Whether to include the built-in encoders')->defaultTrue()->end()->end()->end()->validate()->ifTrue(fn($v) => isset($v['default']))->thenInvalid('"default" is a reserved name.')->end()->end()->end()->validate()->ifTrue(fn($v) => $this->debug && class_exists(JsonParser::class) && !isset($v['default_context'][JsonDecode::DETAILED_ERROR_MESSAGES]))->then(function ($v) {
            $v['default_context'][JsonDecode::DETAILED_ERROR_MESSAGES] = \true;
            return $v;
        })->end()->end()->end();
    }
    ...
    {
        $rootNode->children()->arrayNode('property_info')->info('Property info configuration')->{$enableIfStandalone('symfony/property-info', PropertyInfoExtractorInterface::class)}()->children()->booleanNode('with_constructor_extractor')->info('Registers the constructor extractor.')->end()->end()->end()->end()->validate()->ifTrue(fn($v) => $v['property_info']['enabled'] && !isset($v['property_info']['with_constructor_extractor']))->then(function ($v) {
    ...
    {
        $rootNode->children()->arrayNode('type_info')->info('Type info configuration')->{$enableIfStandalone('symfony/type-info', Type::class)}()->addDefaultsIfNotSet()->children()->arrayNode('aliases', 'alias')->info('Additional type aliases to be used during type context creation.')->defaultValue([])->normalizeKeys(\false)->useAttributeAsKey('name')->scalarPrototype()->end()->end()->end()->end()->end();
    }
    private function addCacheSection(ArrayNodeDefinition $rootNode, callable $willBeAvailable): void
    {
        $rootNode->children()->arrayNode('cache')->info('Cache configuration')->addDefaultsIfNotSet()->children()->scalarNode('prefix_seed')->info('Used to namespace cache keys when using several apps with the same shared backend.')->defaultValue('_%kernel.project_dir%.%kernel.container_class%')->example('my-application-name/%kernel.environment%')->end()->scalarNode('app')->info('App related cache pools configuration.')->defaultValue('cache.adapter.filesystem')->end()->scalarNode('system')->info('System related cache pools configuration.')->defaultValue('cache.adapter.system')->end()->scalarNode('directory')->defaultValue('%kernel.share_dir%/pools/app')->end()->scalarNode('default_psr6_provider')->end()->scalarNode('default_redis_provider')->defaultValue('redis://localhost')->end()->scalarNode('default_valkey_provider')->defaultValue('valkey://localhost')->end()->scalarNode('default_memcached_provider')->defaultValue('memcached://localhost')->end()->scalarNode('default_doctrine_dbal_provider')->defaultValue('database_connection')->end()->scalarNode('default_pdo_provider')->defaultValue($willBeAvailable('doctrine/dbal', Connection::class) && class_exists(DoctrineAdapter::class) ? 'database_connection' : null)->end()->arrayNode('pools', 'pool')->useAttributeAsKey('name')->prototype('array')->validate()->ifTrue(static fn($v) => isset($v['provider']) && 1 < \count($v['adapters']))->thenInvalid('Pool cannot have a "provider" while more than one adapter is defined')->end()->children()->arrayNode('adapters', 'adapter')->performNoDeepMerging()->info('One or more adapters to chain for creating the pool, defaults to "cache.app".')->acceptAndWrap(['string'])->beforeNormalization()->ifArray()->then(static function ($values) {
    ...
    private function addLockSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('lock')->info('Lock configuration')->acceptAndWrap(['string'], 'resources')->{$enableIfStandalone('symfony/lock', Lock::class)}()->beforeNormalization()->ifArray()->then(static function ($v) {
            ...
            return $v;
        })->end()->addDefaultsIfNotSet()->validate()->ifTrue(static fn($v) => $v['enabled'] && !$v['resources'])->thenInvalid('At least one resource must be defined.')->end()->children()->arrayNode('resources', 'resource')->normalizeKeys(\false)->useAttributeAsKey('name')->defaultValue(['default' => [class_exists(SemaphoreStore::class) && SemaphoreStore::isSupported() ? 'semaphore' : 'flock']])->acceptAndWrap(['string'], 'default')->beforeNormalization()->ifArray()->then(static function ($v) {
            if (!array_is_list($v)) {
                return $v;
            }
            $resources = [];
        ...
        })->end()->prototype('array')->performNoDeepMerging()->acceptAndWrap(['string'])->beforeNormalization()->ifNull()->then(static fn() => ['null'])->end()->prototype('scalar')->end()->end()->end()->end()->end()->end();
    }
    private function addSemaphoreSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('semaphore')->info('Semaphore configuration')->acceptAndWrap(['string'], 'resources')->{$enableIfStandalone('symfony/semaphore', Semaphore::class)}()->beforeNormalization()->ifArray()->then(static function ($v) {
            if (!isset($v['resources']) && !isset($v['resource'])) {
                $v = ['resources' => $v];
                if (\array_key_exists('enabled', $v['resources'])) {
                    $v['enabled'] = $v['resources']['enabled'];
            ...
            return $v;
        })->end()->addDefaultsIfNotSet()->children()->arrayNode('resources', 'resource')->normalizeKeys(\false)->useAttributeAsKey('name')->requiresAtLeastOneElement()->acceptAndWrap(['string'], 'default')->beforeNormalization()->ifArray()->then(static function ($v) {
            if (!array_is_list($v)) {
                return $v;
            }
            $resources = [];
    ...
    {
        $rootNode->children()->arrayNode('messenger')->info('Messenger configuration')->{$enableIfStandalone('symfony/messenger', MessageBusInterface::class)}()->validate()->ifTrue(fn($v) => isset($v['buses']) && \count($v['buses']) > 1 && null === $v['default_bus'])->thenInvalid('You must specify the "default_bus" if you define more than one bus.')->end()->validate()->ifTrue(fn($v) => isset($v['buses']) && null !== $v['default_bus'] && !isset($v['buses'][$v['default_bus']]))->then(fn($v) => throw new InvalidConfigurationException(\sprintf('The specified default bus "%s" is not configured. Available buses are "%s".', $v['default_bus'], implode('", "', array_keys($v['buses'])))))->end()->children()->arrayNode('routing')->normalizeKeys(\false)->useAttributeAsKey('message_class')->beforeNormalization()->ifArray()->then(function ($config) {
            ...
            return $newConfig;
        })->end()->prototype('array')->performNoDeepMerging()->children()->arrayNode('senders')->requiresAtLeastOneElement()->prototype('scalar')->end()->end()->end()->end()->end()->arrayNode('serializer')->addDefaultsIfNotSet()->children()->scalarNode('default_serializer')->defaultValue('messenger.transport.native_php_serializer')->info('Service id to use as the default serializer for the transports.')->end()->arrayNode('symfony_serializer')->addDefaultsIfNotSet()->children()->scalarNode('format')->defaultValue('json')->info('Serialization format for the messenger.transport.symfony_serializer service (which is not the serializer used by default).')->end()->arrayNode('context')->normalizeKeys(\false)->useAttributeAsKey('name')->defaultValue([])->info('Context array for the messenger.transport.symfony_serializer service (which is not the serializer used by default).')->prototype('variable')->end()->end()->end()->end()->end()->end()->arrayNode('transports', 'transport')->normalizeKeys(\false)->useAttributeAsKey('name')->arrayPrototype()->acceptAndWrap(['string'], 'dsn')->children()->scalarNode('dsn')->end()->scalarNode('serializer')->defaultNull()->info('Service id of a custom serializer to use.')->end()->arrayNode('options', 'option')->normalizeKeys(\false)->defaultValue([])->prototype('variable')->end()->end()->scalarNode('failure_transport')->defaultNull()->info('Transport name to send failed messages to (after all retries have failed).')->end()->arrayNode('retry_strategy')->addDefaultsIfNotSet()->acceptAndWrap(['string'], 'service')->beforeNormalization()->ifArray()->then(static function ($v) {
        ...
        })->end()->scalarPrototype()->end()->end()->scalarNode('default_bus')->defaultNull()->end()->arrayNode('buses', 'bus')->defaultValue(['messenger.bus.default' => ['default_middleware' => ['enabled' => \true, 'allow_no_handlers' => \false, 'allow_no_senders' => \true], 'middleware' => []]])->normalizeKeys(\false)->useAttributeAsKey('name')->arrayPrototype()->addDefaultsIfNotSet()->children()->arrayNode('default_middleware')->beforeNormalization()->ifString()->then(static fn($v) => ['enabled' => 'allow_no_handlers' === $v, 'allow_no_handlers' => 'allow_no_handlers' === $v])->end()->beforeNormalization()->ifTrue()->then(static fn() => ['enabled' => \true])->end()->beforeNormalization()->ifFalse()->then(static fn() => ['enabled' => \false])->end()->canBeDisabled()->children()->booleanNode('allow_no_handlers')->defaultFalse()->end()->booleanNode('allow_no_senders')->defaultTrue()->end()->end()->end()->arrayNode('middleware')->performNoDeepMerging()->acceptAndWrap(['string'])->beforeNormalization()->ifArray()->then(static fn($v) => \is_string(key($v)) ? [$v] : $v)->end()->defaultValue([])->arrayPrototype()->acceptAndWrap(['string'], 'id')->beforeNormalization()->ifArray()->then(static function ($middleware): array {
        ...
        })->end()->children()->scalarNode('id')->isRequired()->cannotBeEmpty()->end()->arrayNode('arguments', 'argument')->normalizeKeys(\false)->defaultValue([])->prototype('variable')->end()->end()->end()->end()->end()->end()->end()->end()->end()->end();
    }
    ...
    {
        $rootNode->children()->arrayNode('http_client')->info('HTTP Client configuration')->{$enableIfStandalone('symfony/http-client', HttpClient::class)}()->beforeNormalization()->ifArray()->then(static function ($config) {
            ...
            foreach ($config['scoped_clients'] as &$scopedConfig) {
                if ($hasDefaultRateLimiter) {
                    if (!isset($scopedConfig['rate_limiter']) || \true === $scopedConfig['rate_limiter']) {
    ...
    private function createHttpClientRetrySection(): ArrayNodeDefinition
    {
        ...
        return $root->arrayNode('retry_failed')->canBeEnabled()->addDefaultsIfNotSet()->beforeNormalization()->ifArray()->then(static function ($v) {
            ...
            return $v;
        })->end()->children()->scalarNode('retry_strategy')->defaultNull()->info('service id to override the retry strategy.')->end()->arrayNode('http_codes', 'http_code')->performNoDeepMerging()->acceptAndWrap(['int', 'string'])->beforeNormalization()->ifArray()->then(static function ($v) {
            $list = [];
            ...
            return $list;
        })->end()->useAttributeAsKey('code')->arrayPrototype()->children()->integerNode('code')->end()->arrayNode('methods', 'method')->acceptAndWrap(['string'])->beforeNormalization()->ifArray()->then(static fn($v) => array_map('strtoupper', $v))->end()->stringPrototype()->end()->info('A list of HTTP methods that triggers a retry for this status code. When empty, all methods are retried.')->end()->end()->end()->info('A list of HTTP status code that triggers a retry.')->end()->integerNode('max_retries')->defaultValue(3)->min(0)->end()->integerNode('delay')->defaultValue(1000)->min(0)->info('Time in ms to delay (or the initial value when multiplier is used).')->end()->floatNode('multiplier')->defaultValue(2)->min(1)->info('If greater than 1, delay will grow exponentially for each retry: delay * (multiple ^ retries).')->end()->integerNode('max_delay')->defaultValue(0)->min(0)->info('Max time in ms that a retry should ever be delayed (0 = infinite).')->end()->floatNode('jitter')->defaultValue(0.1)->min(0)->max(1)->info('Randomness in percent (between 0 and 1) to apply to the delay.')->end()->end();
    }
    private function addMailerSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('mailer')->info('Mailer configuration')->{$enableIfStandalone('symfony/mailer', Mailer::class)}()->validate()->ifTrue(fn($v) => isset($v['dsn']) && \count($v['transports']))->thenInvalid('"dsn" and "transports" cannot be used together.')->end()->children()->scalarNode('message_bus')->defaultNull()->info('The message bus to use. Defaults to the default bus if the Messenger component is installed.')->end()->scalarNode('dsn')->defaultNull()->end()->arrayNode('transports', 'transport')->useAttributeAsKey('name')->prototype('scalar')->end()->end()->arrayNode('envelope')->info('Mailer Envelope configuration')->children()->scalarNode('sender')->end()->arrayNode('recipients', 'recipient')->performNoDeepMerging()->acceptAndWrap(['string'])->beforeNormalization()->ifArray()->then(static fn($v) => array_values(array_filter($v)))->end()->prototype('scalar')->end()->end()->arrayNode('allowed_recipients', 'allowed_recipient')->info('A list of regular expressions that allow recipients when "recipients" option is defined.')->example(['.*@example\.com'])->performNoDeepMerging()->acceptAndWrap(['string'])->beforeNormalization()->ifArray()->then(static fn($v) => array_values(array_filter($v)))->end()->prototype('scalar')->end()->end()->end()->end()->arrayNode('headers', 'header')->normalizeKeys(\false)->useAttributeAsKey('name')->prototype('array')->normalizeKeys(\false)->acceptAndWrap(['string'], 'value')->beforeNormalization()->ifArray()->then(static fn($v) => array_keys($v) !== ['value'] ? ['value' => $v] : $v)->end()->children()->variableNode('value')->end()->end()->end()->end()->arrayNode('dkim_signer')->addDefaultsIfNotSet()->canBeEnabled()->info('DKIM signer configuration')->children()->scalarNode('key')->info('Key content, or path to key (in PEM format with the `file://` prefix)')->defaultValue('')->cannotBeEmpty()->end()->scalarNode('domain')->defaultValue('')->end()->scalarNode('select')->defaultValue('')->end()->scalarNode('passphrase')->info('The private key passphrase')->defaultValue('')->end()->arrayNode('options', 'option')->performNoDeepMerging()->normalizeKeys(\false)->useAttributeAsKey('name')->prototype('variable')->end()->end()->end()->end()->arrayNode('smime_signer')->addDefaultsIfNotSet()->canBeEnabled()->info('S/MIME signer configuration')->children()->scalarNode('key')->info('Path to key (in PEM format)')->defaultValue('')->cannotBeEmpty()->end()->scalarNode('certificate')->info('Path to certificate (in PEM format without the `file://` prefix)')->defaultValue('')->cannotBeEmpty()->end()->scalarNode('passphrase')->info('The private key passphrase')->defaultNull()->end()->scalarNode('extra_certificates')->defaultNull()->end()->integerNode('sign_options')->defaultNull()->end()->end()->end()->arrayNode('smime_encrypter')->addDefaultsIfNotSet()->canBeEnabled()->info('S/MIME encrypter configuration')->children()->scalarNode('repository')->info('S/MIME certificate repository service. This service shall implement the `Symfony\Component\Mailer\EventListener\SmimeCertificateRepositoryInterface`.')->defaultValue('')->cannotBeEmpty()->end()->integerNode('cipher')->info('A set of algorithms used to encrypt the message')->defaultNull()->beforeNormalization()->ifString()->then(static function ($v): ?int {
        ...
        })->thenInvalid('You must provide a valid cipher.')->end()->end()->end()->end()->end()->end()->end();
    }
    private function addNotifierSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('notifier')->info('Notifier configuration')->{$enableIfStandalone('symfony/notifier', Notifier::class)}()->children()->scalarNode('message_bus')->defaultNull()->info('The message bus to use. Defaults to the default bus if the Messenger component is installed.')->end()->arrayNode('chatter_transports', 'chatter_transport')->useAttributeAsKey('name')->prototype('scalar')->end()->end()->arrayNode('texter_transports', 'texter_transport')->useAttributeAsKey('name')->prototype('scalar')->end()->end()->booleanNode('notification_on_failed_messages')->defaultFalse()->end()->arrayNode('channel_policy')->useAttributeAsKey('name')->prototype('array')->acceptAndWrap(['string'])->prototype('scalar')->end()->end()->end()->arrayNode('admin_recipients', 'admin_recipient')->prototype('array')->children()->scalarNode('email')->cannotBeEmpty()->end()->scalarNode('phone')->defaultValue('')->end()->end()->end()->end()->end()->end()->end();
    }
    ...
    {
        $rootNode->children()->arrayNode('rate_limiter')->info('Rate limiter configuration')->{$enableIfStandalone('symfony/rate-limiter', TokenBucketLimiter::class)}()->beforeNormalization()->ifArray()->then(static function ($v) {
            ...
            return $v;
        })->end()->children()->arrayNode('limiters', 'limiter')->useAttributeAsKey('name')->arrayPrototype()->children()->scalarNode('lock_factory')->info('The service ID of the lock factory used by this limiter (or null to disable locking).')->defaultValue('auto')->end()->scalarNode('cache_pool')->info('The cache pool to use for storing the current limiter state.')->defaultValue('cache.rate_limiter')->end()->scalarNode('storage_service')->info('The service ID of a custom storage implementation, this precedes any configured "cache_pool".')->defaultNull()->end()->enumNode('policy')->info('The algorithm to be used by this limiter.')->isRequired()->values(['fixed_window', 'token_bucket', 'sliding_window', 'compound', 'no_limit'])->end()->arrayNode('limiters', 'limiter')->info('The limiter names to use when using the "compound" policy.')->acceptAndWrap(['string'])->scalarPrototype()->end()->end()->integerNode('limit')->info('The maximum allowed hits in a fixed interval or burst.')->end()->scalarNode('interval')->info('Configures the fixed interval if "policy" is set to "fixed_window" or "sliding_window". The value must be a number followed by "second", "minute", "hour", "day", "week" or "month" (or their plural equivalent).')->end()->arrayNode('rate')->info('Configures the fill rate if "policy" is set to "token_bucket".')->children()->scalarNode('interval')->info('Configures the rate interval. The value must be a number followed by "second", "minute", "hour", "day", "week" or "month" (or their plural equivalent).')->end()->integerNode('amount')->info('Amount of tokens to add each interval.')->defaultValue(1)->end()->end()->end()->end()->validate()->ifTrue(static fn($v) => !\in_array($v['policy'], ['no_limit', 'compound'], \true) && !isset($v['limit']))->thenInvalid('A limit must be provided when using a policy different than "compound" or "no_limit".')->end()->end()->end()->end()->end()->end();
    }
    private function addUidSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('uid')->info('Uid configuration')->{$enableIfStandalone('symfony/uid', UuidFactory::class)}()->addDefaultsIfNotSet()->children()->enumNode('default_uuid_version')->values([7, 6, 4, 1])->defaultValue(7)->end()->enumNode('name_based_uuid_version')->defaultValue(5)->values([5, 3])->end()->scalarNode('name_based_uuid_namespace')->cannotBeEmpty()->end()->enumNode('time_based_uuid_version')->values([7, 6, 1])->defaultValue(7)->end()->scalarNode('time_based_uuid_node')->cannotBeEmpty()->end()->end()->end()->end();
    }
    private function addHtmlSanitizerSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('html_sanitizer')->info('HtmlSanitizer configuration')->{$enableIfStandalone('symfony/html-sanitizer', HtmlSanitizerInterface::class)}()->children()->arrayNode('sanitizers', 'sanitizer')->useAttributeAsKey('name')->arrayPrototype()->children()->booleanNode('allow_safe_elements')->info('Allows "safe" elements and attributes.')->defaultFalse()->end()->booleanNode('allow_static_elements')->info('Allows all static elements and attributes from the W3C Sanitizer API standard.')->defaultFalse()->end()->arrayNode('allow_elements', 'allow_element')->info('Configures the elements that the sanitizer should retain from the input. The element name is the key, the value is either a list of allowed attributes for this element or "*" to allow the default set of attributes (https://wicg.github.io/sanitizer-api/#default-configuration).')->example(['i' => '*', 'a' => ['title'], 'span' => 'class'])->normalizeKeys(\false)->useAttributeAsKey('name')->variablePrototype()->beforeNormalization()->ifArray()->then(static fn($n) => $n['attribute'] ?? $n)->end()->validate()->ifTrue(fn($n): bool => !\is_string($n) && !\is_array($n))->thenInvalid('The value must be either a string or an array of strings.')->end()->end()->end()->arrayNode('block_elements', 'block_element')->info('Configures elements as blocked. Blocked elements are elements the sanitizer should remove from the input, but retain their children.')->acceptAndWrap(['string'])->stringPrototype()->end()->end()->arrayNode('drop_elements', 'drop_element')->info('Configures elements as dropped. Dropped elements are elements the sanitizer should remove from the input, including their children.')->acceptAndWrap(['string'])->stringPrototype()->end()->end()->arrayNode('allow_attributes', 'allow_attribute')->info('Configures attributes as allowed. Allowed attributes are attributes the sanitizer should retain from the input.')->normalizeKeys(\false)->useAttributeAsKey('name')->variablePrototype()->beforeNormalization()->ifArray()->then(static fn($n) => $n['element'] ?? $n)->end()->end()->end()->arrayNode('drop_attributes', 'drop_attribute')->info('Configures attributes as dropped. Dropped attributes are attributes the sanitizer should remove from the input.')->normalizeKeys(\false)->useAttributeAsKey('name')->variablePrototype()->beforeNormalization()->ifArray()->then(static fn($n) => $n['element'] ?? $n)->end()->end()->end()->arrayNode('force_attributes', 'force_attribute')->info('Forcefully set the values of certain attributes on certain elements.')->normalizeKeys(\false)->useAttributeAsKey('name')->arrayPrototype()->normalizeKeys(\false)->useAttributeAsKey('name')->stringPrototype()->end()->end()->end()->booleanNode('force_https_urls')->info('Transforms URLs using the HTTP scheme to use the HTTPS scheme instead.')->defaultFalse()->end()->arrayNode('allowed_link_schemes', 'allowed_link_scheme')->info('Allows only a given list of schemes to be used in links href attributes.')->acceptAndWrap(['string'])->stringPrototype()->end()->end()->arrayNode('allowed_link_hosts', 'allowed_link_host')->info('Allows only a given list of hosts to be used in links href attributes.')->defaultNull()->acceptAndWrap(['string'])->stringPrototype()->end()->end()->booleanNode('allow_relative_links')->info('Allows relative URLs to be used in links href attributes.')->defaultFalse()->end()->arrayNode('allowed_media_schemes', 'allowed_media_scheme')->info('Allows only a given list of schemes to be used in media source attributes (img, audio, video, ...).')->acceptAndWrap(['string'])->stringPrototype()->end()->end()->arrayNode('allowed_media_hosts', 'allowed_media_host')->info('Allows only a given list of hosts to be used in media source attributes (img, audio, video, ...).')->defaultNull()->acceptAndWrap(['string'])->stringPrototype()->end()->end()->booleanNode('allow_relative_medias')->info('Allows relative URLs to be used in media source attributes (img, audio, video, ...).')->defaultFalse()->end()->arrayNode('with_attribute_sanitizers', 'with_attribute_sanitizer')->info('Registers custom attribute sanitizers.')->acceptAndWrap(['string'])->stringPrototype()->end()->end()->arrayNode('without_attribute_sanitizers', 'without_attribute_sanitizer')->info('Unregisters custom attribute sanitizers.')->acceptAndWrap(['string'])->stringPrototype()->end()->end()->integerNode('max_input_length')->info('The maximum length allowed for the sanitized input.')->defaultValue(0)->end()->end()->end()->end()->end()->end()->end();
    }
    private function addJsonStreamerSection(ArrayNodeDefinition $rootNode, callable $enableIfStandalone): void
    {
        $rootNode->children()->arrayNode('json_streamer')->info('JSON streamer configuration')->{$enableIfStandalone('symfony/json-streamer', StreamWriterInterface::class)}()->end()->end();
    }
}
