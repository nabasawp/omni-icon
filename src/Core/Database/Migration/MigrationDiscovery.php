...
final class MigrationDiscovery implements Discovery
{
    ...
    public function discover(DiscoveryLocation $discoveryLocation, ClassReflector $classReflector): void
    {
        ...
        if (!class_exists($className) || !is_subclass_of($className, \OmniIcon\Core\Database\Migration\MigrationInterface::class)) {
            $this->logger->warning("Class has Migration attribute but doesn't implement MigrationInterface", ['component' => LogComponent::MIGRATION_DISCOVERY, 'className' => $className]);
            return;
        }
        $timestamp = \OmniIcon\Core\Database\Migration\MigrationRegistry::extractTimestampFromClassName($className);
        $this->discoveryItems->add($discoveryLocation, ['className' => $className, 'version' => $timestamp, 'description' => $migrationAttribute->description, 'priority' => $migrationAttribute->priority]);
    }
    public function apply(): void
    {
        ...
        foreach ($this->discoveryItems as $discoveryItem) {
            ...
            self::$migrations[$discoveryItem['className']] = $discoveryItem;
            // Use className as key to prevent duplicates
        }
