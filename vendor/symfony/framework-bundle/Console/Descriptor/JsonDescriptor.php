...
class JsonDescriptor extends Descriptor
{
    ...
    protected function describeContainerTags(ContainerBuilder $container, array $options = []): void
    {
        ...
        foreach ($this->findDefinitionsByTag($container, $showHidden) as $tag => $definitions) {
            ...
            foreach ($definitions as $definition) {
                $data[$tag][] = $this->getContainerDefinitionData($definition, \true, $container, $options['id'] ?? null);
            }
        }
        $this->writeData($data, $options);
    }
    ...
    protected function describeContainerAlias(Alias $alias, array $options = [], ?ContainerBuilder $container = null): void
    {
        ...
        $this->writeData([$this->getContainerAliasData($alias), $this->getContainerDefinitionData($container->getDefinition((string) $alias), isset($options['omit_tags']) && $options['omit_tags'], $container, (string) $alias)], array_merge($options, ['id' => (string) $alias]));
    }
    ...
    {
        $data = ['class' => (string) $definition->getClass(), 'public' => $definition->isPublic(), 'synthetic' => $definition->isSynthetic(), 'lazy' => $definition->isLazy(), 'shared' => $definition->isShared(), 'abstract' => $definition->isAbstract(), 'autowire' => $definition->isAutowired(), 'autoconfigure' => $definition->isAutoconfigured()];
    ...
    {
        return ['service' => (string) $alias, 'public' => $alias->isPublic()];
    }
