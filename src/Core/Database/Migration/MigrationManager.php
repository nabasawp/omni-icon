...
{
    public function __construct(private readonly \OmniIcon\Core\Database\Migration\MigrationRegistry $migrationRegistry, private readonly \OmniIcon\Core\Database\Migration\MigrationRepository $migrationRepository, private readonly \OmniIcon\Core\Database\Migration\MigrationRunner $migrationRunner)
    {
    }
    /**
     * @return array{
    ...
    public function getStatus(): array
    {
        ...
        foreach ($allMigrations as $allMigration) {
            $isExecuted = in_array($allMigration['className'], $executedVersions, \true);
            $executionRecord = $executionData[$allMigration['className']] ?? null;
            $migrations[] = ['version' => $allMigration['version'], 'fqcn' => $allMigration['className'], 'description' => $allMigration['description'], 'status' => $isExecuted ? 'executed' : 'pending', 'execution_time' => $executionRecord['execution_time'] ?? null, 'executed_at' => $executionRecord['executed_at'] ?? null];
        }
        return ['total' => count($allMigrations), 'executed' => count($executedVersions), 'pending' => count($allMigrations) - count($executedVersions), 'migrations' => $migrations];
    }
    /**
