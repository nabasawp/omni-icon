...
use OmniIconDeps\Twig\ExpressionParser\ExpressionParserType;
...
use OmniIconDeps\Twig\Node\BodyNode;
...
use OmniIconDeps\Twig\Node\MacroNode;
use OmniIconDeps\Twig\Node\ModuleNode;
use OmniIconDeps\Twig\Node\Node;
use OmniIconDeps\Twig\Node\NodeCaptureInterface;
use OmniIconDeps\Twig\Node\NodeOutputInterface;
use OmniIconDeps\Twig\Node\Nodes;
use OmniIconDeps\Twig\Node\PrintNode;
use OmniIconDeps\Twig\Node\TextNode;
use OmniIconDeps\Twig\TokenParser\TokenParserInterface;
use OmniIconDeps\Twig\Util\ReflectionCallable;
/**
...
class Parser
{
    ...
    public function parse(TokenStream $stream, $test = null, bool $dropNeedle = \false): ModuleNode
    {
        ...
        $node = new ModuleNode(new BodyNode([$body]), $this->parent, $this->blocks ? new Nodes($this->blocks) : new EmptyNode(), $this->macros ? new Nodes($this->macros) : new EmptyNode(), $this->traits ? new Nodes($this->traits) : new EmptyNode(), $this->embeddedTemplates ? new Nodes($this->embeddedTemplates) : new EmptyNode(), $stream->getSourceContext());
    ...
    private function checkPrecedenceDeprecations(ExpressionParserInterface $expressionParser, AbstractExpression $expr)
    {
        ...
        if ($expressionParser instanceof PrefixExpressionParserInterface) {
            ...
            foreach ($precedenceChanges as $ep => $changes) {
                if (!\in_array($expressionParser, $changes, \true)) {
                    continue;
                }
