...
use OmniIconDeps\Symfony\Component\Console\Cursor;
use OmniIconDeps\Symfony\Component\Console\Formatter\OutputFormatterStyle;
use OmniIconDeps\Symfony\Component\Console\Helper\Table;
use OmniIconDeps\Symfony\Component\Console\Helper\TableCell;
use OmniIconDeps\Symfony\Component\Console\Helper\TableCellStyle;
 ...
 * @internal
 */
#[AsCommand(name: 'ux:icons:search', description: 'Search icons and icon sets from ux.symfony.com')]
final class SearchIconCommand extends Command
{
    ...
    {
        $this->addArgument('prefix', InputArgument::REQUIRED, 'Prefix or name of the icon set (ex: bootstrap, fa, tabler)')->addArgument('name', InputArgument::OPTIONAL, 'Name of the icon (leave empty to search for sets)')->setHelp(<<<EOF
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (1 < \count($iconSets) && $prefix !== $iconSet['prefix']) {
            ...
            if (!$choice || \false === $prefix = array_search($choice, $choices, \true)) {
    ...
    private function renderIconSetTable(SymfonyStyle $io, array $iconSets): void
    {
        ...
        foreach ($iconSets as $prefix => $iconSet) {
            $results[] = [$iconSet['name'] ?? $prefix, new TableCell($iconSet['total'], ['style' => new TableCellStyle(['align' => 'right'])]), $iconSet['license']['title'] ?? '', $iconSet['prefix'], $this->formatIcon($io, $prefix . ':' . $iconSet['samples'][0], \false)];
        }
    ...
    private function formatIcon(OutputInterface $output, string $icon, bool $padding = \true): string
    {
