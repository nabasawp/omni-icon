...
use OmniIconDeps\Symfony\Component\Console\Formatter\OutputFormatter;
use OmniIconDeps\Symfony\Component\Console\Helper\Dumper;
use OmniIconDeps\Symfony\Component\Console\Helper\Table;
use OmniIconDeps\Symfony\Component\Console\Helper\TableCell;
...
class TextDescriptor extends Descriptor
{
    ...
    protected function describeRouteCollection(RouteCollection $routes, array $options = []): void
    {
        ...
        foreach ($routes->all() as $name => $route) {
            ...
            $row = ['Name' => $name, 'Methods' => $this->formatMethods($route->getMethods()), 'Scheme' => $scheme, 'Host' => $host, 'Path' => $route->getPath()];
            if ($showControllers) {
    ...
    protected function describeRoute(Route $route, array $options = []): void
    {
        ...
        $tableRows = [['Route Name', $options['name'] ?? ''], ['Path', $route->getPath()], ['Path Regex', $route->compile()->getRegex()], ['Host', '' !== $route->getHost() ? $route->getHost() : 'ANY'], ['Host Regex', '' !== $route->getHost() ? $route->compile()->getHostRegex() : ''], ['Scheme', $route->getSchemes() ? implode('|', $route->getSchemes()) : 'ANY'], ['Method', $this->formatMethods($route->getMethods())], ['Requirements', $route->getRequirements() ? $this->formatRouterConfig($route->getRequirements()) : 'NO CUSTOM'], ['Class', $route::class], ['Defaults', $this->formatRouterConfig($defaults)], ['Options', $this->formatRouterConfig($route->getOptions())]];
    ...
    private function formatMethods(array $methods): string
    {
        ...
        return implode('|', array_map(fn(string $method): string => \sprintf('<fg=%s>%s</>', self::VERB_COLORS[$method] ?? 'default', $method), $methods));
    }
    /**
