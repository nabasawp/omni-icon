 ...
 */
namespace OmniIconDeps\Symfony\Component\HttpKernel\HttpCache;
...
use OmniIconDeps\Symfony\Component\HttpKernel\TerminableInterface;
/**
...
class HttpCache implements HttpKernelInterface, TerminableInterface
{
    ...
    public function terminate(Request $request, Response $response): void
    {
        ...
        if (\in_array('fresh', $this->traces[$this->getTraceKey($request)] ?? [], \true)) {
            return;
        }
    ...
    protected function validate(Request $request, Response $entry, bool $catch = \false): Response
    {
        ...
        if (304 == $response->getStatusCode()) {
            ...
            if ($etag && \in_array($etag, $requestEtags, \true) && !\in_array($etag, $cachedEtags, \true)) {
                return $response;
            }
            $entry = clone $entry;
    ...
    protected function forward(Request $request, bool $catch = \false, ?Response $entry = null): Response
    {
         ...
         */
        if (null !== $entry && \in_array($response->getStatusCode(), [500, 502, 503, 504], \true) && !$entry->headers->hasCacheControlDirective('no-cache') && !$entry->mustRevalidate()) {
