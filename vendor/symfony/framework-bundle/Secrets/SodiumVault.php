 ...
 */
namespace OmniIconDeps\Symfony\Bundle\FrameworkBundle\Secrets;
...
class SodiumVault extends AbstractVault implements EnvVarLoaderInterface
{
     ...
     */
    public function __construct(string $secretsDir, #[\SensitiveParameter] string|\Stringable|null $decryptionKey = null, private ?string $derivedSecretEnvVar = null)
    {
    ...
    public function reveal(string $name): ?string
    {
        ...
        if (!\function_exists('sodium_crypto_box_seal') && !\function_exists('OmniIconDeps\sodium_crypto_box_seal')) {
    ...
    public function loadEnvVars(): array
    {
        ...
        if ($this->derivedSecretEnvVar && !\array_key_exists($this->derivedSecretEnvVar, $envs)) {
            ...
            $envs[$this->derivedSecretEnvVar] = LazyString::fromCallable(static fn() => '' !== ($k = (string) $k) ? base64_encode(hash('sha256', $k, \true)) : '');
        }
        return $envs;
    }
