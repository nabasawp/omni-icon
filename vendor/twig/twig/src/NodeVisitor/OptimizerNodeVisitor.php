...
use OmniIconDeps\Twig\Node\ForNode;
use OmniIconDeps\Twig\Node\IncludeNode;
...
final class OptimizerNodeVisitor implements NodeVisitorInterface
{
    ...
    private function enterOptimizeFor(Node $node): void
    {
        if ($node instanceof ForNode) {
        ...
        } elseif ($node instanceof ContextVariable && \in_array($node->getAttribute('name'), $this->loopsTargets, \true)) {
            $node->setAttribute('always_defined', \true);
            ...
            $this->addLoopToAll();
        } elseif ($node instanceof GetAttrExpression && (!$node->getNode('attribute') instanceof ConstantExpression || 'parent' === $node->getNode('attribute')->getAttribute('value')) && (\true === $this->loops[0]->getAttribute('with_loop') || $node->getNode('node') instanceof ContextVariable && 'loop' === $node->getNode('node')->getAttribute('name'))) {
            $this->addLoopToAll();
        }
    }
    /**
