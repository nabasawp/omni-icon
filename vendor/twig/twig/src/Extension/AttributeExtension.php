...
final class AttributeExtension extends AbstractExtension
{
    ...
    {
        return max(filemtime(__FILE__), is_file($filename = (new \ReflectionClass($this->getClass()))->getFileName()) ? filemtime($filename) : 0);
    }
    private function initFromAttributes(): void
    {
        ...
        foreach ($reflectionClass->getMethods() as $method) {
            ...
            foreach ($method->getAttributes(AsTwigFunction::class) as $reflectionAttribute) {
                ...
                $attribute = $reflectionAttribute->newInstance();
                $callable = new TwigFunction($attribute->name, [$reflectionClass->name, $method->getName()], ['needs_context' => $attribute->needsContext ?? \false, 'needs_environment' => $attribute->needsEnvironment ?? $this->needsEnvironment($method), 'needs_charset' => $attribute->needsCharset ?? \false, 'is_variadic' => $method->isVariadic(), 'is_safe' => $attribute->isSafe, 'is_safe_callback' => $attribute->isSafeCallback, 'deprecation_info' => $attribute->deprecationInfo]);
                if ($callable->getMinimalNumberOfRequiredArguments() > $method->getNumberOfParameters()) {
            ...
            foreach ($method->getAttributes(AsTwigTest::class) as $reflectionAttribute) {
                ...
                $attribute = $reflectionAttribute->newInstance();
                $callable = new TwigTest($attribute->name, [$reflectionClass->name, $method->getName()], ['needs_context' => $attribute->needsContext ?? \false, 'needs_environment' => $attribute->needsEnvironment ?? $this->needsEnvironment($method), 'needs_charset' => $attribute->needsCharset ?? \false, 'is_variadic' => $method->isVariadic(), 'deprecation_info' => $attribute->deprecationInfo]);
                if ($callable->getMinimalNumberOfRequiredArguments() > $method->getNumberOfParameters()) {
    ...
    private function needsEnvironment(\ReflectionFunctionAbstract $function): bool
    {
        ...
        return $parameters[0]->getType() instanceof \ReflectionNamedType && Environment::class === $parameters[0]->getType()->getName() && !$parameters[0]->isVariadic();
    }
}
