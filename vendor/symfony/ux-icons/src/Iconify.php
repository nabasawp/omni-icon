...
use OmniIconDeps\Symfony\UX\Icons\Exception\HttpClientNotInstalledException;
...
final class Iconify
{
    ...
    public function __construct(private CacheInterface $cache, private string $endpoint = self::API_ENDPOINT, private ?HttpClientInterface $httpClient = null, ?int $maxIconsQueryLength = null)
    {
    ...
    public function fetchIcon(string $prefix, string $name): Icon
    {
        ...
        return new Icon($data['icons'][$name]['body'], ['xmlns' => self::ATTR_XMLNS_URL, 'viewBox' => \sprintf('0 0 %s %s', $width ?? $height ?? self::DEFAULT_ICON_WIDTH, $height ?? $width ?? self::DEFAULT_ICON_HEIGHT)]);
    }
    public function fetchIcons(string $prefix, array $names): array
    {
        ...
        foreach ($names as $iconName) {
            ...
            $icons[$iconName] = new Icon($iconData['body'], ['xmlns' => self::ATTR_XMLNS_URL, 'viewBox' => \sprintf('0 0 %d %d', $width ?? $height ?? self::DEFAULT_ICON_WIDTH, $height ?? $width ?? self::DEFAULT_ICON_HEIGHT)]);
        }
        return $icons;
    }
