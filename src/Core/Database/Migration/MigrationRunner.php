...
final class MigrationRunner
{
    ...
    {
        $executedVersions = $this->migrationRepository->getExecutedVersions(); // Now returns FQCNs
        ...
        if ($versions === []) {
            ...
            foreach ($versions as $version) {
                ...
                if (in_array($migration['className'], $executedVersions, true)) {
        ...
        foreach ($migrationsToRun as $migrationToRun) {
            try {
                ...
                $error = [
                    ...
                    'className' => $migrationToRun['className'],
                ...
                return [
                    'executed' => $executed,
                    'failed' => $error,
                    'message' => sprintf('Migration failed: %s - %s', $migrationToRun['className'], $e->getMessage())
                ];
            }
        }
        ...
        foreach ($migrationsToRollback as $migrationToRollback) {
            try {
                ...
                return [
                    ...
                    'failed' => $error,
                    'message' => sprintf('Rollback failed: %s - %s', $migrationToRollback, $e->getMessage())
                ];
            }
        }
    ...
    private function runSingleMigration(array $migrationData, bool $dryRun = false): array
    {
        ...
        if ($dryRun) {
            return [
                ...
                'className' => $migrationData['className'],
        ...
        $this->migrationRepository->markAsExecuted(
