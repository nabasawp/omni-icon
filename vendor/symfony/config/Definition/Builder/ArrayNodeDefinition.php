...
use OmniIconDeps\Symfony\Component\Config\Definition\ArrayNode;
...
use OmniIconDeps\Symfony\Component\Config\Definition\PrototypedArrayNode;
/**
...
{
    protected bool $performDeepMerging = \true;
    protected bool $ignoreExtraKeys = \false;
    protected bool $removeExtraKeys = \true;
    /**
    ...
    protected bool $allowNewKeys = \true;
    ...
    protected bool $normalizeKeys = \true;
    /**
    ...
    {
        $disabledNode = $this->addDefaultsIfNotSet()->treatFalseLike(['enabled' => \false])->treatTrueLike(['enabled' => \true])->treatNullLike(['enabled' => \true])->beforeNormalization()->ifArray()->then(static function ($v) {
            $v['enabled'] ??= \true;
            return $v;
    ...
    {
        $enabledNode = $this->addDefaultsIfNotSet()->treatFalseLike(['enabled' => \false])->treatTrueLike(['enabled' => \true])->treatNullLike(['enabled' => \true])->beforeNormalization()->ifArray()->then(static function ($v) {
            $v['enabled'] ??= \true;
            return $v;
        })->end()->children()->booleanNode('enabled')->defaultTrue();
        $info = 1 <= \func_num_args() ? func_get_arg(0) : null;
        if ($info) {
