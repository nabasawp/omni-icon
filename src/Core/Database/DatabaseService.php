...
use OmniIconDeps\Doctrine\DBAL\Exception;
...
use wpdb;
#[Service]
final class DatabaseService implements \OmniIcon\Core\Database\DatabaseInterface
{
    ...
    public function __construct(wpdb $wpdb)
    {
        ...
        $this->connection = DriverManager::getConnection(['driver' => 'pdo_mysql', 'host' => $host, 'port' => $port, 'dbname' => \DB_NAME, 'user' => \DB_USER, 'password' => \DB_PASSWORD, 'charset' => defined('DB_CHARSET') ? \DB_CHARSET : 'utf8mb4', 'collation' => defined('DB_COLLATE') ? \DB_COLLATE : 'utf8mb4_unicode_ci', 'driverOptions' => [PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8mb4 COLLATE utf8mb4_unicode_ci"]]);
    }
    // Raw SQL operations
    ...
    {
        $queryBuilder = $this->createQueryBuilder()->select('*')->from($this->getTableName($table));
    ...
    {
        $queryBuilder = $this->createQueryBuilder()->select('1')->from($this->getTableName($table))->setMaxResults(1);
        foreach ($where as $column => $value) {
            $queryBuilder->andWhere($queryBuilder->expr()->eq($column, $queryBuilder->createNamedParameter($value)));
        }
