 ...
 */
namespace OmniIconDeps\Symfony\Component\ErrorHandler;

use OmniIconDeps\Composer\InstalledVersions;
use OmniIconDeps\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use OmniIconDeps\Doctrine\Persistence\Proxy;
use OmniIconDeps\Mockery\MockInterface;
use OmniIconDeps\Phake\IMock;
use OmniIconDeps\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use OmniIconDeps\PHPUnit\Framework\MockObject\MockObject;
use OmniIconDeps\PHPUnit\Framework\MockObject\Stub;
use OmniIconDeps\Prophecy\Prophecy\ProphecySubjectInterface;
use OmniIconDeps\ProxyManager\Proxy\ProxyInterface;
use OmniIconDeps\Psr\Log\LogLevel;
use OmniIconDeps\Symfony\Component\DependencyInjection\Argument\LazyClosure;
use OmniIconDeps\Symfony\Component\ErrorHandler\Internal\TentativeTypes;
...
class DebugClassLoader
{
    ...
    private function setReturnType(string $types, string $class, string $method, string $filename, ?string $parent, ?\ReflectionType $returnType = null): void
    {
        ...
        foreach ($typesMap as $n => $t) {
            if ('null' !== $n) {
                $iterable = $iterable && (\in_array($n, ['array', 'iterable'], \true) || str_contains($n, 'Iterator'));
                $object = $object && (\in_array($n, ['callable', 'object', '$this', 'static'], \true) || !isset(self::SPECIAL_RETURN_TYPES[$n]));
            }
        }
        $phpTypes = [];
        $docTypes = [];
        foreach ($typesMap as $n => $t) {
            ...
            if (!isset($phpTypes['']) && !\in_array($n, $phpTypes, \true)) {
                $phpTypes[] = $n;
            }
        }
