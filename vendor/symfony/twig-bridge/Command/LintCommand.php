 ...
 */
namespace OmniIconDeps\Symfony\Bridge\Twig\Command;

use OmniIconDeps\Symfony\Component\Console\Attribute\AsCommand;
use OmniIconDeps\Symfony\Component\Console\CI\GithubActionReporter;
...
class LintCommand extends Command
{
    ...
    public function __construct(private Environment $twig, private array $namePatterns = ['*.twig'])
    {
        parent::__construct();
    }
    protected function configure(): void
    {
        $this->addOption('format', null, InputOption::VALUE_REQUIRED, \sprintf('The output format ("%s")', implode('", "', $this->getAvailableFormatOptions())))->addOption('show-deprecations', null, InputOption::VALUE_NONE, 'Show deprecations as errors')->addArgument('filename', InputArgument::IS_ARRAY, 'A file, a directory or "-" for reading from STDIN')->addOption('excludes', null, InputOption::VALUE_REQUIRED | InputOption::VALUE_IS_ARRAY, 'Excluded directories', [])->setHelp(<<<'EOF'
...
EOF
    ...
    private function validate(string $template, string $file, bool $collectDeprecation): array
    {
        ...
        return ['template' => $template, 'file' => $file, 'deprecations' => $deprecations, 'valid' => \true];
    }
    private function display(InputInterface $input, OutputInterface $output, SymfonyStyle $io, array $files): int
    {
        return match ($this->format) {
            ...
            'github' => $this->displayTxt($output, $io, $files, \true),
