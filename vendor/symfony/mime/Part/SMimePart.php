...
class SMimePart extends AbstractPart
{
    ...
    public function __unserialize(array $data): void
    {
        ...
        $p = "\x00" . self::class . "\x00";
        if (["\x00*\x00_headers", $p . 'body', $p . 'type', $p . 'subtype', $p . 'parameters'] === array_keys($data)) {
            $r = new \ReflectionProperty(parent::class, 'headers');
            $r->setValue($this, $data["\x00*\x00_headers"]);
            $this->body = $data[$p . 'body'];
            $this->type = $data[$p . 'type'];
            $this->subtype = $data[$p . 'subtype'];
            $this->parameters = $data[$p . 'parameters'];
            if ($wakeup) {
                $this->_headers = $data["\x00*\x00_headers"];
                $this->__wakeup();
            }
            return;
        }
