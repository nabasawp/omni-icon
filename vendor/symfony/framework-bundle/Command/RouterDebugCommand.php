...
use OmniIconDeps\Symfony\Bundle\FrameworkBundle\Console\Helper\DescriptorHelper;
...
class RouterDebugCommand extends Command
{
    ...
    public function __construct(private RouterInterface $router, private ?FileLinkFormatter $fileLinkFormatter = null)
    {
        parent::__construct();
    }
    protected function configure(): void
    {
        $this->setDefinition([new InputArgument('name', InputArgument::OPTIONAL, 'A route name'), new InputOption('show-controllers', null, InputOption::VALUE_NONE, 'Show assigned controllers in overview'), new InputOption('show-aliases', null, InputOption::VALUE_NONE, 'Show aliases in overview'), new InputOption('format', null, InputOption::VALUE_REQUIRED, \sprintf('The output format ("%s")', implode('", "', $this->getAvailableFormatOptions())), 'txt'), new InputOption('raw', null, InputOption::VALUE_NONE, 'To output raw route(s)'), new InputOption('method', null, InputOption::VALUE_REQUIRED, 'Filter by HTTP method', '', ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'])])->setHelp(<<<'EOF'
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($name) {
            ...
            if (!$input->isInteractive() && !$route && \count($matchingRoutes) > 1) {
                $helper->describe($io, $this->findRouteContaining($name, $routes), ['format' => $input->getOption('format'), 'raw_text' => $input->getOption('raw'), 'show_controllers' => $input->getOption('show-controllers'), 'show_aliases' => $input->getOption('show-aliases'), 'output' => $io, 'method' => $method]);
                return 0;
            }
            ...
            $helper->describe($io, $routes, ['format' => $input->getOption('format'), 'raw_text' => $input->getOption('raw'), 'show_controllers' => $input->getOption('show-controllers'), 'show_aliases' => $input->getOption('show-aliases'), 'output' => $io, 'container' => $container, 'method' => $method]);
        }
        return 0;
    }
    private function findRouteNameContaining(string $name, RouteCollection $routes, string $method): array
    {
        ...
        foreach ($routes as $routeName => $route) {
            if (\false !== stripos($routeName, $name) && (!$method || !$route->getMethods() || \in_array($method, $route->getMethods(), \true))) {
