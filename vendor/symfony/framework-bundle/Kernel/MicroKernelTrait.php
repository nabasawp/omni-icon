 ...
 */
namespace OmniIconDeps\Symfony\Bundle\FrameworkBundle\Kernel;
...
use OmniIconDeps\Symfony\Component\DependencyInjection\Loader\Configurator\AbstractConfigurator;
use OmniIconDeps\Symfony\Component\DependencyInjection\Loader\Configurator\ContainerConfigurator;
use OmniIconDeps\Symfony\Component\DependencyInjection\Loader\PhpFileLoader as ContainerPhpFileLoader;
use OmniIconDeps\Symfony\Component\DependencyInjection\Reference;
use OmniIconDeps\Symfony\Component\Routing\Loader\Configurator\RoutingConfigurator;
use OmniIconDeps\Symfony\Component\Routing\Loader\PhpFileLoader as RoutingPhpFileLoader;
...
trait MicroKernelTrait
{
    ...
    protected function getKernelParameters(): array
    {
        ...
        $bundlesDefinition = !is_file($bundlesPath) ? [FrameworkBundle::class => ['all' => \true]] : require $bundlesPath;
        $knownEnvs = [$this->environment => \true];
    ...
    {
        if ('' !== $dir && \in_array($dir[0], ['/', '\\'], \true)) {
            return $dir . '/' . $this->environment;
        }
        if ('\\' === \DIRECTORY_SEPARATOR && ':' === ($dir[1] ?? '') && 65 <= \ord($dir[0]) && \ord($dir[0]) <= 122 && !\in_array($dir[0], ['[', ']', '^', '_', '`'], \true)) {
            return $dir . '/' . $this->environment;
        }
